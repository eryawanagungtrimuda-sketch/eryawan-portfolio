...
export async function getPublishedProjectBySlug(slug: string) {
  ...
  const { data, error } = await supabase
    .from('projects')
    .select(`${projectColumns},project_images(id,project_id,image_url,alt_text,sort_order,area_tags,display_ratio,object_position,crop_x,crop_y,crop_zoom,created_at)`)
    .eq('slug', slug)
    .eq('is_published', true)
