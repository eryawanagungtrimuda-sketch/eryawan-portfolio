...
export default function InsightForm({ insight, projects = [], initialImages = [] }: { insight?: Insight | null; projects?: { id: string; title: string }[]; initialImages?: InsightImage[] }) {
  ...
  return (<form onSubmit={onSubmit} className="space-y-5 rounded-2xl border border-white/10 bg-white/[0.02] p-4 sm:p-5 md:p-6">
    ...
    <div className="flex gap-3"><button type="submit" disabled={loading || generating} className="rounded-lg bg-[#E5A900] px-4 py-2 text-sm text-black disabled:opacity-60">{loading ? 'Menyimpan...' : 'Simpan Perubahan'}</button><button type="button" onClick={generateDraft} disabled={!canGenerate || loading || generating} className="rounded-lg border border-white/20 px-4 py-2 text-sm disabled:opacity-60">{generating ? 'Memproses AI...' : 'Bangun / Perbaiki dengan AI'}</button></div>
  </form>);
}
