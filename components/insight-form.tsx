...
export default function InsightForm({ insight, projects = [] }: { insight?: Insight | null; projects?: { id: string; title: string }[] }) {
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-5 rounded-2xl border border-white/10 bg-white/[0.02] p-5 md:p-6">
      ...
      <div className="space-y-1.5"><label className="text-sm">URL Cover Image</label><input className="w-full rounded-lg border border-white/15 bg-transparent px-3 py-2" value={form.cover_image} onChange={(e) => setForm({ ...form, cover_image: e.target.value })} /></div>
      ...
      <div className="flex flex-wrap gap-3">
        <button type="submit" disabled={loading || generating} className="rounded-lg bg-[#E5A900] px-4 py-2 text-sm font-medium text-black disabled:opacity-60">{loading ? 'Menyimpan...' : insight ? 'Simpan Perubahan' : 'Simpan Draft'}</button>
        <button type="button" onClick={generateDraft} disabled={!canGenerate || loading || generating} className="rounded-lg border border-white/20 px-4 py-2 text-sm disabled:opacity-60">{generating ? 'Memproses AI...' : 'Bangun / Perbaiki dengan AI'}</button>
      </div>
    </form>
  );
}
