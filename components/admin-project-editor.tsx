    ...
    async function loadProject() {
      ...
      try {
        ...
        const { data, error, status } = await supabase
          .from('projects')
          .select(
            'id,title,slug,category,design_category,design_style,area_type,area_tags,cover_image,problem,solution,impact,konteks,konflik,keputusan_desain,pendekatan,dampak,insight_kunci,project_status,completion_year,client_problem_raw,design_reference,area_scope,project_size,is_published,created_at,project_images(id,project_id,image_url,alt_text,sort_order,area_tags,display_ratio,object_position,created_at)'
          )
          .eq('id', id)
          .maybeSingle();
