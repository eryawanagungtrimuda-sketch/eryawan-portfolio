        ...
        {options.map((item) => {
          ...
          return (
            <button
              ...
              className={`rounded-full border px-3.5 py-2 font-mono text-[10px] font-black uppercase tracking-[0.14em] transition duration-300 ${
                active
...
function Badge({ children }: { children?: string | null }) {
  ...
  return (
    <span className="rounded-full border border-white/10 px-3 py-1 font-mono text-[10px] font-black uppercase tracking-[0.14em] text-white/50">
      {children}
    </span>
  );
}
  ...
  return (
    <section className="pb-24">
      <div className="rounded-sm border border-white/10 bg-white/[0.018] p-5 md:p-7 lg:p-8">
        ...
        <div className="mt-8 grid gap-6 border-t border-white/10 pt-7 md:mt-9 md:grid-cols-2 md:gap-7 lg:grid-cols-3 lg:pt-8">
        ...
        <div className="mt-12 grid gap-8 md:grid-cols-2 xl:grid-cols-3">
          {filteredProjects.map((project, index) => (
            <article key={project.id} className="group relative flex h-full flex-col overflow-hidden rounded-sm border border-white/12 bg-gradient-to-br from-white/[0.035] to-white/[0.012] p-5 md:p-6">
              {project.cover_image ? (
                <div className="mb-6 aspect-[16/10] overflow-hidden rounded-sm border border-white/10 bg-white/[0.02]">
                ...
                <div className="mb-6 flex aspect-[16/10] items-center justify-center rounded-sm border border-white/10 bg-white/[0.025] text-center text-sm text-white/32">
              ...
              <div className="mt-6 space-y-5 border-t border-white/10 pt-6">
                <div>
                  ...
                  <p className="mt-2.5 text-sm leading-[1.7] text-white/68 md:text-base">{project.category || project.design_category || 'Kategori project belum ditentukan.'}</p>
                </div>
                <div>
                  <p className="font-mono text-[10px] font-black uppercase tracking-[0.26em] text-white/45">Problem</p>
                  <p className="mt-2.5 text-sm leading-[1.7] text-white/68 md:text-base">{truncateText(project.problem || 'Masalah ruang belum didefinisikan.', 110)}</p>
                </div>
                <div>
                  <p className="font-mono text-[10px] font-black uppercase tracking-[0.26em] text-white/45">Decision / Solusi</p>
                  <p className="mt-2.5 text-sm leading-[1.7] text-white/68 md:text-base">{truncateText(project.solution || 'Keputusan desain difokuskan pada prioritas fungsi, alur ruang, dan efisiensi implementasi.', 110)}</p>
                </div>
                <div>
                  <p className="font-mono text-[10px] font-black uppercase tracking-[0.26em] text-white/45">Impact</p>
                  <p className="mt-2.5 text-sm leading-[1.7] text-white/68 md:text-base">{truncateText(project.impact || 'Ruang menjadi lebih terarah, efisien, dan mudah digunakan.', 110)}</p>
                </div>
              </div>

              <Link
                href={`/karya/${project.slug}`}
                className="mt-8 inline-flex items-center gap-3 rounded-sm border border-[#D4AF37] bg-[#D4AF37] px-5 py-3 font-mono text-xs font-black uppercase tracking-[0.18em] text-black transition hover:bg-[#E2C866]"
              >
