      ...
      <div ref={resultsRef}>
        {filteredProjects.length === 0 ? <div className="mt-10 flex min-h-[260px] items-center justify-center rounded-2xl border border-white/10 bg-white/[0.018] p-8 text-center"><p className="max-w-md text-lg leading-8 text-white/66">Tidak ada karya yang sesuai dengan filter ini.</p></div> : (
          <div className="mt-12 grid gap-7 md:grid-cols-2 xl:grid-cols-3">
            ...
            return <article key={project.id} style={{ '--reveal-delay': `${index * 100}ms` } as CSSProperties} className={`reveal-on-scroll mobile-card-breathing mobile-reveal-card premium-card-hover group relative flex h-full flex-col overflow-hidden rounded-2xl border bg-gradient-to-br from-white/[0.035] via-white/[0.02] to-black/25 transition motion-safe:duration-500 motion-safe:ease-out hover:bg-white/[0.04] hover:shadow-[0_26px_58px_rgba(0,0,0,0.36)] ${index === 0 ? 'border-[#D4AF37]/55 md:col-span-2 xl:col-span-2' : 'border-white/12 hover:border-[#D4AF37]/35'}`}>
