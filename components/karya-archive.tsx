...
function Badge({ children }: { children?: string | null }) {
  ...
  return <span className="rounded-full border border-white/[0.08] bg-white/[0.018] px-3 py-1.5 font-sans text-[10px] font-semibold uppercase tracking-[0.12em] text-white/58">{children}</span>;
}
      ...
      <div ref={resultsRef}>
        {filteredProjects.length === 0 ? <div className="mt-10 flex min-h-[260px] items-center justify-center rounded-2xl border border-white/10 bg-white/[0.018] p-8 text-center"><div><p className="max-w-md text-lg leading-8 text-white/66">Belum ada karya yang cocok dengan kombinasi filter ini.</p><button type="button" onClick={resetFilters} className="mt-5 min-h-11 rounded-full border border-[#D4AF37]/40 px-4 py-2 font-sans text-sm font-semibold text-[#D4AF37]">Reset Filter</button></div></div> : (
          <div className="mt-12 grid gap-7 md:grid-cols-2 xl:grid-cols-3">
            ...
            return <article key={project.id} style={{ '--reveal-delay': `${index * 90}ms`, '--premium-card-border': index === 0 ? 'rgba(212, 175, 55, 0.42)' : 'rgba(255, 255, 255, 0.105)' } as CSSProperties} className={`reveal-on-scroll mobile-card-breathing mobile-card-reveal premium-oval-card premium-oval-frame group relative flex h-full flex-col border border-transparent bg-gradient-to-br from-white/[0.035] via-[#15120b]/[0.18] to-black/30 shadow-[0_18px_46px_rgba(0,0,0,0.18)] transition-all motion-safe:duration-500 motion-safe:ease-out hover:-translate-y-1 hover:bg-white/[0.045] hover:shadow-[0_22px_52px_rgba(0,0,0,0.28)] ${index === 0 ? 'md:col-span-2 xl:col-span-2' : ''}`}>
              {thumbnail.imageUrl ? (
