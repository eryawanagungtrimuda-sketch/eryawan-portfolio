    ...
    <MobileSwipeRow ariaLabel="Metrik dampak dapat digeser horizontal" desktopGridClassName="lg:grid-cols-2 lg:gap-4" backgroundTone="#142030">
      {impacts.map((impact, index) => (
        ...
        >
          ...
          <h3 className="mt-4 shrink-0 font-sans text-[1.05rem] font-semibold leading-snug tracking-[-0.01em] text-white/90 md:text-[1.15rem]">
            {impact.title}
          </h3>
          <p className="mt-3 max-w-md flex-1 font-sans text-sm leading-6 text-white/60 md:text-[0.95rem]">{impact.description}</p>
          <span className="mt-5 block h-1.5 w-16 shrink-0 rounded-full bg-gradient-to-r from-[#C8A951]/70 to-[#C8A951]/15 transition-all duration-300 group-hover:w-24" />
        </article>
      ))}
    </MobileSwipeRow>
  );
