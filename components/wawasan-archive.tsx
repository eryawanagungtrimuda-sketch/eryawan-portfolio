...
export default function WawasanArchive({ insights }: Props) {
  ...
  const applyDraft = () => {
    setCategory(draftCategory); setContentType(draftContentType); setSourceType(draftSourceType); setSort(draftSort); setIsFilterOpen(false);
  };
    ...
    <section className="reveal-on-scroll mobile-scroll-section mobile-section-breathing mx-auto mt-8 max-w-7xl pb-28 sm:mt-10">
      <div style={{ '--premium-card-border': 'rgba(255, 255, 255, 0.10)' } as CSSProperties} className="premium-oval-card premium-oval-frame relative border border-transparent bg-white/[0.02] p-4 sm:p-5">
        ...
        <div className="grid grid-cols-[minmax(0,1fr)_96px] gap-2 sm:gap-3">
          ...
          <button type="button" onClick={() => setIsFilterOpen((v) => !v)} aria-expanded={isFilterOpen} aria-controls="wawasan-filter-panel" className="premium-interactive h-11 rounded-2xl border border-[#C8A951]/40 px-2 font-sans text-sm font-semibold text-[#D4AF37] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#D4AF37]/60">
        ...
        <div className={`absolute left-4 right-4 top-[calc(100%+8px)] z-40 hidden rounded-2xl border border-white/15 bg-[#11100d] p-4 shadow-[0_18px_50px_rgba(0,0,0,0.45)] md:block ${isFilterOpen ? 'md:block' : 'md:hidden'}`} id="wawasan-filter-panel" role="dialog" aria-label="Filter Wawasan desktop">
      ...
      {filteredInsights.length === 0 ? <div style={{ '--premium-card-border': 'rgba(255, 255, 255, 0.10)' } as CSSProperties} className="premium-oval-card premium-oval-frame mt-6 border border-transparent bg-white/[0.02] p-8 text-center"><p className="font-sans text-white/70">Belum ada wawasan yang cocok dengan pencarian atau filter saat ini. Coba atur ulang filter atau gunakan kata kunci yang lebih umum.</p><button onClick={resetAll} className="premium-interactive mt-4 rounded-xl border border-[#D4AF37]/40 px-4 py-2 font-sans text-sm text-[#D4AF37] active:translate-y-0 active:scale-[0.98]">Atur Ulang Filter</button></div> : <>
        {featured ? <article style={{ '--reveal-delay': '0ms', '--premium-card-border': 'rgba(200, 169, 81, 0.25)' } as CSSProperties} className="reveal-on-scroll mobile-card-breathing mobile-card-reveal premium-oval-card-lg premium-oval-frame mt-6 border border-transparent bg-gradient-to-b from-[#1a160a] to-[#0c0b08] md:grid md:grid-cols-2">{featured.cover_image ? <div className="premium-oval-media"><img src={featured.cover_image} alt={featured.title} className="h-full w-full object-cover" loading="lazy" decoding="async" /></div> : null}<div className="p-6 md:p-8"><span className="rounded-full border border-[#C8A951]/40 bg-[#C8A951]/10 px-3 py-1 font-mono text-[11px] uppercase tracking-[0.12em] text-[#D4AF37]">{toLabel(featured.category)}</span><h2 className="mt-4 font-sans text-3xl leading-tight">{featured.title}</h2><p className="mt-3 font-sans text-white/70">{featured.excerpt || 'Pilihan wawasan utama untuk membantu Anda mengambil keputusan desain yang lebih tepat.'}</p><Link href={`/wawasan/${featured.slug}`} className="premium-interactive mt-6 inline-flex min-h-11 items-center rounded-full border border-[#D4AF37]/40 px-5 font-mono text-sm uppercase tracking-[0.12em] text-[#D4AF37] active:translate-y-0 active:scale-[0.98]">Baca Wawasan Lengkap</Link></div></article> : null}
        <div className="mt-8 grid gap-5 sm:gap-6 md:grid-cols-2 xl:grid-cols-3">{rest.map((item, index) => <div key={item.id} style={{ '--reveal-delay': `${(index + 1) * 90}ms` } as CSSProperties} className="reveal-on-scroll mobile-card-breathing mobile-card-reveal"><InsightCard item={item} frameColor="rgba(255, 255, 255, 0.10)" /></div>)}</div>
      </>}

      {isFilterOpen && createPortal(<div className="fixed inset-0 z-[9999] md:hidden"><button aria-label="Tutup filter" className="absolute inset-0 bg-black/70" onClick={() => setIsFilterOpen(false)} /><div id="wawasan-filter-panel" role="dialog" aria-modal="true" aria-label="Filter Wawasan" className="absolute bottom-0 left-0 right-0 mx-auto flex max-h-[78dvh] w-full flex-col rounded-t-[26px] border border-white/10 bg-[#11100d]"><div className="sticky top-0 z-10 rounded-t-[26px] border-b border-white/10 bg-[#11100d] px-4 pb-3 pt-2"><div className="mx-auto mb-2 h-1 w-10 rounded-full bg-white/25" /><div className="flex items-start justify-between gap-3"><div><h3 className="font-sans text-sm font-semibold text-white">Filter Wawasan</h3><p className="mt-1 font-sans text-xs text-white/58">Pilih topik, jenis konten, sumber, dan urutan wawasan.</p></div><button aria-label="Tutup filter wawasan" onClick={() => setIsFilterOpen(false)} className="min-h-10 rounded-full border border-white/20 px-3 font-sans text-xs text-white/80">Tutup</button></div></div><div className="overflow-y-auto px-4 py-4"><div className="space-y-4"><div><p className="mb-2 font-sans text-xs font-semibold uppercase tracking-[0.13em] text-white/55">Topik</p><FilterChips options={categoryOptions} value={draftCategory} onSelect={setDraftCategory} /></div><div><p className="mb-2 font-sans text-xs font-semibold uppercase tracking-[0.13em] text-white/55">Jenis Konten</p><FilterChips options={contentTypeOptions} value={draftContentType} onSelect={setDraftContentType} /></div><div><p className="mb-2 font-sans text-xs font-semibold uppercase tracking-[0.13em] text-white/55">Sumber</p><FilterChips options={sourceTypeOptions} value={draftSourceType} onSelect={setDraftSourceType} kind="source" /></div><div><label htmlFor="sort" className="mb-2 block font-sans text-xs font-semibold uppercase tracking-[0.13em] text-white/55">Urutkan</label><select id="sort" value={draftSort} onChange={(e) => setDraftSort(e.target.value as SortType)} className="min-h-11 w-full rounded-2xl border border-white/10 bg-[#090909] px-4 py-2 font-sans text-sm text-white/72 outline-none focus:border-[#D4AF37]/40"><option value="terbaru">Terbaru</option><option value="terlama">Terlama</option><option value="judul_az">Judul A-Z</option></select></div></div></div><div className="sticky bottom-0 mt-auto flex gap-2 border-t border-white/10 bg-[#11100d] px-4 pb-[calc(env(safe-area-inset-bottom)+0.9rem)] pt-3"><button onClick={() => { setDraftCategory(ALL); setDraftContentType(ALL); setDraftSourceType(ALL); setDraftSort('terbaru'); }} className="min-h-11 flex-1 rounded-full border border-white/20 px-4 py-2 font-sans text-sm font-semibold text-white/80">Reset</button><button onClick={applyDraft} className="min-h-11 flex-1 rounded-full border border-[#D4AF37]/50 bg-[#D4AF37]/10 px-4 py-2 font-sans text-sm font-semibold text-[#D4AF37]">Terapkan</button></div></div></div>, document.body)}
