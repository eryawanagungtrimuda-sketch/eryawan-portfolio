...
export async function POST(request: Request) {
  ...
  const nama = sanitize(payload.nama, 160), perusahaan = sanitize(payload.perusahaan, 160), email = sanitize(payload.email, 160), whatsapp = sanitize(payload.whatsapp, 80), jenisKebutuhan = sanitize(payload.jenisKebutuhan, 160), lokasiProject = sanitize(payload.lokasiProject, 160), estimasiLuas = sanitize(payload.estimasiLuas, 120), tahapProject = sanitize(payload.tahapProject, 160), timeline = sanitize(payload.timeline, 120), budgetRange = sanitize(payload.budgetRange, 120), kebutuhanUtama = sanitize(payload.kebutuhanUtama, 3000), statusFile = sanitize(payload.statusFile, 160), messagePreview = sanitize(payload.messagePreview, 6000);
